...
PUBLIC int do_unlink()
{
	...
	byte_idx = bit_idx / (u32)8;
	int bits_left = pin->i_nr_sects;
	int byte_cnt = (bits_left - (8 - (bit_idx % 8))) / (u32)8;

	/* current sector nr. */
	int s = 2  /* 2: bootsect + superblk */
		+ sb->nr_imap_sects + byte_idx / (u32)SECTOR_SIZE;
	...
	int nr_dir_blks = (dir_inode->i_size + SECTOR_SIZE) / (u32)SECTOR_SIZE;
	int nr_dir_entries =
		dir_inode->i_size / (u32)DIR_ENTRY_SIZE; /* including unused slots
						     * (the file has been
	...
	for (i = 0; i < nr_dir_blks; i++) {
		...
		int j;
		for (j = 0; j < SECTOR_SIZE / (u32)DIR_ENTRY_SIZE; j++,pde++) {
			if (++m > nr_dir_entries)
				break;
